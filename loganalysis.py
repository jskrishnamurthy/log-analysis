...
popularArticles_sql = """select Replace(path,'/article/','') as Article_Name,
                        ...
                        where path <> '/' group by path order by count(*) desc limit 3;"""
...
failurePercentage_sql = """select ErrorLogs.logTimestamp as Error_Date,
                           cast((ErrorLogs.logCount::decimal/(ErrorLogs.logCount::decimal+ SuccessLogs.logCount::decimal)) * 100 as decimal(18,2)) as FailPercentage 
                           from ErrorLogs join SuccessLogs 
                           on ErrorLogs.logTimestamp = SuccessLogs.logTimestamp 
                           where cast((ErrorLogs.logCount::decimal/(ErrorLogs.logCount::decimal+ SuccessLogs.logCount::decimal)) * 100 as decimal(18,2)) > 1;"""
